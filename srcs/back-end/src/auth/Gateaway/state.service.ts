...
export class StateService {
    ...
    async invite(client : Socket,id : number)
    {
            try {
                ...
                const bloqued = await this.prismaService.blockedU.findFirst({where : {
                    userBloqued : invited.id,
                    user : {
                        id : user
                    }
                }})
    ...
    async afk(client : Socket){
        try {
            let user;
                ...
                await this.prismaService.user.findFirstOrThrow({where : {id: user}})
                await this.prismaService.user.update({where : {id : user},data : {state : 'Afk'}})
        } catch (error) {
            console.log(error)
        }
    }
